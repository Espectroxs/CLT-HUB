...
local label = farmPlusTab:AddLabel("Farmming Rock")
...
-- Config rocks (nome + durability necessária)
local rockList = {
    ["Tiny Island Rock"] = 0,
...
-- Variável global para seleção
getgenv().selectedRock = nil
getgenv().autoFarm = false

-- Dropdown para selecionar pedra
local dropdown = farmPlusTab:AddDropdown("Select Rock", function(text)
    getgenv().selectedRock = text
end)

-- Adiciona todas as opções no dropdown
for rockName, _ in pairs(rockList) do
    dropdown:Add(rockName)
end

-- Switch para ligar/desligar
local switch = farmPlusTab:AddSwitch("Auto Rock", function(Value)
    ...
    task.spawn(function()
        while getgenv().autoFarm do
            ...
            -- Verifica se tem pedra selecionada
            if not getgenv().selectedRock then continue end

            local durabilityNeeded = rockList[getgenv().selectedRock]
            local durability = game:GetService("Players").LocalPlayer.Durability.Value
...
switch:Set(false) -- começa desligado 

local label = farmPlusTab:AddLabel("Auto Training")
label.Font = Enum.Font.FredokaOne
label.TextSize = 25

getgenv().selectedTraining = nil
getgenv().autoTraining = false
getgenv().autoLift = false

-- Lista de ferramentas de treino + evento
local trainingTools = {
    ["Weight"] = "rep",
    ["Pushups"] = "rep",
    ["Handstands"] = "rep",
...
-- Dropdown para selecionar treino
local dropdown = farmPlusTab:AddDropdown("Select Training", function(text)
    getgenv().selectedTraining = text
end)

-- Adiciona as opções
for toolName, _ in pairs(trainingTools) do
    dropdown:Add(toolName)
end

-- Switch Auto Training
local switch = farmPlusTab:AddSwitch("Auto Training", function(Value)
    getgenv().autoTraining = Value

    task.spawn(function()
        while getgenv().autoTraining do
            task.wait(0.001)

            if not getgenv().selectedTraining then continue end
            local player = game.Players.LocalPlayer
            local char = player.Character
            local toolName = getgenv().selectedTraining
...
-- Switch Auto Lift (separado, só para Weight)
local autoLiftSwitch = farmPlusTab:AddSwitch("Auto Lift Weight", function(Value)
    ...
    task.spawn(function()
        while getgenv().autoLift do
            ...
            if weightTool and char and char:FindFirstChild("Humanoid") then
...
autoLiftSwitch:Set(false)

farmPlusTab:AddSwitch("Fast Tools", function(state)
    fastTools = state
    task.spawn(function()
        while fastTools do
            task.wait()
            if LocalPlayer.Character then
                for _, tool in pairs(LocalPlayer.Backpack:GetChildren()) do
                    if tool:IsA("Tool") then
                        tool.Grip = CFrame.new(0,0,0) -- mais rápido
                    end
                end
            end
        end
...
local rbTab = window:AddTab("Rebirths")

local rebirthFarmFolder = rbTab:AddFolder("Premium Farm")
...
local rebirthsFolder = rbTab:AddFolder("Rebirths")
...
rebirthsFolder:AddTextBox("Rebirth Target", function(text)
    ...
    if newValue and newValue > 0 then
        ...
        game:GetService("StarterGui"):SetCore("SendNotification", {
            ...
            Text = "Congratulations: " .. tostring(targetRebirthValue) .. " Rebirths",
            Duration = 0
        })
    else
        game:GetService("StarterGui"):SetCore("SendNotification", {
...
local targetSwitch = rebirthsFolder:AddSwitch("Rebirth Target", function(bool)
...
misc:AddButton("Dark Mode", function()
    ...
    if darkModeEnabled then
        ...
        print("Modo Escuro Ativado (Night)")
    else
        ...
        l.ClockTime = originalSettings.ClockTime
        ...
        print("Modo Escuro Desativado (ClockTime restaurado)")
    end
end)

-- Botão para Gamepass AutoLift
misc:AddButton("Free AutoLift Gamepass", function()
...
local label1 = noteTab:AddLabel("AGRADEÇO A DEUS POR TUDO EM MINHA VIDA")
label1.Font = Enum.Font.GothamBold
label1.TextSize = 20

local label2 = noteTab:AddLabel("Obrigado, Senhor, pela minha família, meus amigos, minha saúde,")
label2.Font = Enum.Font.GothamBold
label2.TextSize = 20

local label3 = noteTab:AddLabel("Minhas conquistas e pelas oportunidades que me dá a cada dia.")
label3.Font = Enum.Font.GothamBold
label3.TextSize = 20

local label4 = noteTab:AddLabel("Que eu nunca me esqueça de Te honrar e confiar em Teus planos.")
label4.Font = Enum.Font.GothamBold
label4.TextSize = 20

local label5 = noteTab:AddLabel("Glória a Deus por cada vitória, cada aprendizado e cada momento de paz.")
